...
async def test_park_drains_delivery_or_preserves_uncertainty(tmp_path, monkeypatch, acknowledged):
    ...
    runner._relay_delivery_drain_timeout_secs = cleanup_budget
...
@pytest.mark.asyncio
async def test_delivered_reply_can_finish_native_hooks_beyond_disconnect_budget(tmp_path, monkeypatch):
    from gateway import delivery_ledger as ledger
    ...
    policy = ConversationPolicy({'source': source.to_dict(), 'conversation_key': build_session_key(source),
                                 'allowed_user_ids': ['owner']})
    completion_started, release, disconnected = asyncio.Event(), asyncio.Event(), asyncio.Event()
    outcomes = []

    class Transport:
        async def send_outbound(self, action, **kwargs):
            return {'success': True, 'message_id': 'final-reply'}

        async def disconnect(self, **kwargs):
            disconnected.set()

    class DelayedCompletion(ConversationRelayAdapter):
    ...
    descriptor = CapabilityDescriptor(contract_version=1, platform='discord', label='Discord',
        max_message_length=2000, supports_draft_streaming=False, supports_edit=True,
        supports_threads=True, markdown_dialect='discord', len_unit='chars')
    adapter = DelayedCompletion(PlatformConfig(), descriptor, Transport(), policy)
    adapter.config.typing_indicator = False
    adapter.set_message_handler(lambda event: asyncio.sleep(0, result='Finished.'))
    runner = object.__new__(ConversationGateway)
    runner._exit_code = 0
    runner._exit_reason = None
    runner.adapters = {Platform.RELAY: adapter}
    runner._adapter_disconnect_timeout_secs = lambda: 0.02
    runner._relay_delivery_drain_timeout_secs = lambda: 2.0
    await adapter.handle_message(MessageEvent(text='work', source=source, message_id='input'))
    await asyncio.wait_for(completion_started.wait(), 2)
    with ledger._connect() as db:
        assert [r[0] for r in db.execute('SELECT state FROM delivery_obligations')] == ['delivered']
    teardown = asyncio.create_task(runner._bounded_adapter_teardown(adapter, Platform.RELAY))
    # A healthy completion hook can outlast transport disconnection's own budget.
    await asyncio.sleep(0.1)
    assert not teardown.done() and not disconnected.is_set()
    release.set()
    await asyncio.wait_for(teardown, 2)
    assert outcomes == [ProcessingOutcome.SUCCESS]
    assert runner.exit_code == 0 and disconnected.is_set()
    assert ConversationGateway._relay_delivery_drain_timeout_secs(runner) == 30.0
