...
class Controller:
    def __init__(self, config: dict, backend, knowledge, *, connector_factory=None):
        ...
        self.connector = connector_factory(config, self.route, self.prompt_response, ledger=self.ledger,
                                           media=self.media, relay_url=config["relay_url"])
        self.connections = {}
    ...
    async def provision(self, owner):
        ...
        self.ledger.transition(owner["id"], owner["generation"], "provisioning", identity=asdict(identity),
                               detail="Pod created; waiting for the retained worker to connect")
                ...
                async with self.operations:
                    ...
                    while owner := self.ledger.reserve_next():
                        try:
                            ...
                            self.ledger.transition(owner["id"], owner["generation"], "recovery_required",
                                                   detail=f"provisioning needs inspection: {type(exc).__name__}")
    ...
    async def route(self, event):
        ...
        audience = await self.connector.audience(event)
        owner = self.ledger.get(cid)
        if event.text.strip().startswith("/cluster "):
    ...
    async def prompt_response(self, prompt_id, option_id, interaction):
        ...
        if await self.connector.audience(row["source"]) != row["audience"]:
            self.ledger.transition(row["id"], row["generation"], "recovery_required", detail="audience changed during approval")
            raise OwnershipError("destination audience changed")
    ...
    async def cluster_command(self, owner, event):
        ...
        if str(event.source.user_id) != owner["actor"] or await self.connector.audience(event) != owner["audience"]:
